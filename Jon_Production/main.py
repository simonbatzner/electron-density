        ...
        if n == 0:
            ...
            if par != 1:
                ...
                for atom in range(noa):
                    for p in range(3):
                          processes.append(mp.Process(target=pred_comp, args=(pos_curr, atom, cutoff, eta_lower, \
                                                              eta_upper, eta_length, brav_mat, brav_inv, \
                                                              vec1, vec2, vec3, p, db, gp, force_conv, proc_cnt, output)))
                          proc_cnt += 1
                ...
                # collect and store as dict
                results = [output.get() for _ in processes]
                results_dict = {}

                for result in results:
                    results_dict.update({result[0]: [result[1], [result[2]]]})
                ...
                for m in range(noa):
                    ...
                    for p in range(3):
                        force_pred, err_pred = results_dict[index][0], results_dict[index][1]
                        index += 1
