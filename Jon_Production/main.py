...
from Jon_Production.utility import *
...
for n in range(frames):
    ...
    if n == 0:
        ...
        pos_text, cell, pos, supercell, pos_label = get_perturbed_pos(unit_cell, dim, unit_pos, pert_size)
        pos_store.append(pos)

        # call QE
        scf_text = get_scf_input(pos_text, cell, pseudo_dir, outdir, unit_cell, unit_pos, ecut, nk, dim, nat, pert_size)
        ...
        aug_and_norm(pos, forces, db, cutoff, eta_lower, eta_upper, eta_length, brav_mat, brav_inv, vec1, vec2, vec3)
        ...
        update_txt += 'average force is ' + str(np.mean(np.abs(np.array(forces) * force_conv))) + ' eV/A.\n'
        ...
        if par != 1:
            ...
            for m in range(noa):
                ...
                atom = m
                symm = symmetrize_forces(pos_curr, atom, cutoff, eta_lower, eta_upper, eta_length,
