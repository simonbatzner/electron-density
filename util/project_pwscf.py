...
#def parse_qe_pwscf_md_output(path):
	...
	for n in range(len(step_chunks)):
		
		# Figure out how many lines are invovled
		chunk_length = len(step_chunks[n])
		current_chunk = step_chunks[n]
		...
		toten = current_chunk[0].split('=')[-1].strip().split()[0]
		...
		#print(forces,positions,elements,temperature,time,iteration,toten)
